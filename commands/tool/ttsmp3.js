...
module.exports = {
    ...
    code: async (ctx) => {
        ...
        const input = ctx.args.slice(availableModels.includes(ctx.args[0]?.toLowerCase()) ? 1 : 0).join(" ") || ctx?.quoted?.conversation || (ctx.quoted && ((Object.values(ctx.quoted).find(v => v?.text || v?.caption)?.text) || (Object.values(ctx.quoted).find(v => v?.text || v?.caption)?.caption))) || null;
