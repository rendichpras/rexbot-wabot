...
async function get(type) {
    try {
        ...
        const createList = (data) => {
            const items = data.map(d => global.formatter.quote(d));
            return `${items.join(`\n${global.formatter.quote("─────")}\n`)}\n\n${global.config.msg.footer}`;
        };

        switch (type) {
            case "alkitab": {
                const data = (await axios.get(api.createUrl("https://api-alkitab.vercel.app", "/api/book"))).data.data;
                text = createList(data.map(d =>
            ...
            default: {
                text = global.formatter.quote(`❎ Tipe tidak diketahui: ${type}`);
                break;
            }
        }
        ...
        global.consolefy.error(`Error: ${util.format(error)}`);
        return global.formatter.quote("❎ Terjadi kesalahan saat mengambil daftar.");
    }
}
